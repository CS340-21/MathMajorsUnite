...
class Read(object):
    ...
    def read_files(self, dirname, num=1):
        # IF ONLY ONE FILE
        if num == 1:
...
#        return os.path.abspath(exe)
    ...
    def regress(self, filename, name):
        ...
        for index, item in enumerate(lr.coef_):
            out1 += " + {}*{}".format('{:.2f}'.format(item), x.columns[index])
        out2 = "\nModel generated has an R^2 of {} on the given data!".format('{:.2f}'.format(lr.score(x,y)))

        #return out

        out_dict = []
        out_dict.append(('intercept', '{:.5f}'.format(lr.intercept_)))
        for index, item in enumerate(lr.coef_):
            out_dict.append((x.columns[index], '{:.5f}'.format(item)))
        
        return out1, out2, out_dict, '{:.3f}'.format(lr.score(x, y))
